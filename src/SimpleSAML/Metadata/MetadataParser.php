...
class MetadataParser
{
    ...
    /**
     * This is the constructor for the MetadataParser class.
     *
     * @param EntityDescriptor $entityElement The EntityDescriptor.
     ...
     *
     * @return \SimpleSAML\Metadata\MetadataParser An instance of this class with the metadata loaded.
    ...
    public static function parseFile(string $file): MetadataParser
    {
     ...
     */
    public static function parseString(string $metadata): MetadataParser
    {
        try {
     ...
     * @return \SimpleSAML\Metadata\MetadataParser An instance of this class with the metadata loaded.
     */
    public static function parseDocument(DOMDocument $document): MetadataParser
    {
     ...
     * @return \SimpleSAML\Metadata\MetadataParser An instance of this class with the metadata loaded.
     */
    public static function parseElement(EntityDescriptor $entityElement): MetadataParser
    {
        return new MetadataParser($entityElement, null, []);
    }


    /**
     * This function parses a file where the root node is either an EntityDescriptor element or an
     * EntitiesDescriptor element. In both cases it will return an associative array of MetadataParser instances. If
     * the file contains a single EntityDescriptorElement, then the array will contain a single MetadataParser
     * instance.
     *
     ...
     *
     * @return \SimpleSAML\Metadata\MetadataParser[] An array of MetadataParser instances.
     * @throws \Exception If the file does not parse as XML.
     */
    #[\Deprecated('Will be removed in 3.0. No replacement was suggested', '16-12-2025')]
     ...
     *
     * @return \SimpleSAML\Metadata\MetadataParser[] An associative array of MetadataParser instances.
     ...
     * EntitiesDescriptor element. It will return an associative array of MetadataParser instances in both cases.
     *
    ...
    public static function parseDescriptorsElement(?DOMElement $element = null): array
    {
        ...
        if ($xmlUtils->isDOMNodeOfType($element, 'EntityDescriptor', '@md') === true) {
            ...
            $entityDescriptor = EntityDescriptor::fromXML($element);
    ...
    private static function processDescriptorsElement(
        ...
        if ($element instanceof EntityDescriptor) {
            $ret = new MetadataParser($element, $maxExpireTime, $validators, $parentExtensions);
            $ret = [$ret->getEntityId()->getValue() => $ret];
            /** @var \SimpleSAML\Metadata\MetadataParser[] $ret */
            return $ret;
        }
     ...
     *
     * @param AbstractRoleDescriptorType $element The element we should extract metadata from.
     ...
     */
    private static function parseRoleDescriptorType(AbstractRoleDescriptorType $element, ?int $expireTime): array
    {
        $ret = [];
     ...
     * - 'keys': Array of associative arrays with the elements from parseKeyDescriptor:
     *
     * @param AbstractSSODescriptor $element The element we should extract metadata from.
     * @param int|null                       $expireTime The unix timestamp for when this element should expire, or
     *                             NULL if unknown.
     *
     * @return array An associative array with metadata we have extracted from this element.
     */
    private static function parseSSODescriptor(AbstractSSODescriptor $element, ?int $expireTime): array
    {
...
//        }
    ...
    private static function processExtensions(mixed $element, array $parentExtensions = []): array
    {
        ...
        $elist = $element->getExtensions()?->getElements();
        if( !$elist ) {
            $elist = [];
        }
        foreach ($elist as $e) {
     ...
     *
     * @return EntityDescriptor The \DOMEntity which represents the EntityDescriptor.
