...
class SQLPersistentNameID extends BaseNameIDGenerator
{
    ...
    protected function getValue(array &$state): ?string
    {
        ...
        if (!isset($state['Destination']['entityid'])) {
            $this->logger->warning('SQLPersistentNameID: No SP entity ID - not generating persistent NameID.');
            return null;
        }
        $spEntityId = $state['Destination']['entityid'];

        if (!isset($state['Source']['entityid'])) {
            $this->logger->warning('SQLPersistentNameID: No IdP entity ID - not generating persistent NameID.');
            return null;
        }
