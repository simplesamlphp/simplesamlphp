...
class ServiceProvider
{
    ...
    public function login(Request $request, string $sourceId): RunnableResponse
    {
        ...
        $paramList = ['ReturnTo' => true];
        $spSource = $as->getAuthSource();

        if ($spSource->isRequestInitiation()) {
            $paramList = [
                // The location of a resource to which the user agent should be returned,
                // when possible, following successful authentication. If this parameter
                // is omitted, the service provider MUST use a default value
                // (which it unilaterally determines)
                'target' => false,
                // The unique identifier of an identity provider the service provider is
                // instructed to use. If it cannot or will not do so, the service provider
                // MUST NOT return a request for authentication to a different
                // identity provider (i.e., it MUST NOT ignore the choice).
                // If this parameter is omitted, the service provider is free to respond
                // in any fashion it wishes, including but not limited to the use of any
                // supported discovery mechanism to determine the identity provider itself.
                'entityID' => false,
                // A boolean value of "true" or "false" that indicates whether the request generated by the service
                // provider should include an option to prevent visible user interaction with the identity provider.
                // This corresponds to the SAML 2.0 IsPassive attribute in a <samlp:AuthnRequest> message.
                // If this parameter is present and "true", and the authentication protocol supported by the service
                // provider and identity provider in common does not support this feature, then the service provider
                // MUST redirect the user agent to the value of the target parameter.
                'isPassive' => false,
                // A boolean value of "true" or "false" that indicates whether the request generated by the service
                // provider should include an option to bypass an existing security context and require explicit user
                // interaction during authentication to the identity provider. This corresponds to the SAML 2.0
                // ForceAuthn attribute in a <samlp:AuthnRequest> message.
                // If this parameter is present and "true", and the authentication protocol supported by the service
                // provider and identity provider in common does not support this feature, then the service provider
                // MUST NOT return a request for authentication.
                'forceAuthn' => false,
                ...$paramList,
            ];
        }
        ...
        $parsedParams = $this->parseRequestParameters($request, $paramList);
        // Map the Request parameters to options
        $options = $this->mapToOptionsList($parsedParams);

        $this->handleLogin($as, $options);

        return new RunnableResponse([new Utils\HTTP(), 'redirectTrustedURL'], [$options['ReturnTo']]);
    }
    ...
    public function discoResponse(Request $request): RunnableResponse
    {
        ...
        $parsedParams = $this->parseRequestParameters($request, $params);

        $state = $this->authState::loadState($parsedParams['AuthID'], 'saml:sp:sso');
        ...
        return new RunnableResponse([$source, 'startSSO'], [$parsedParams['idpentityid'], $state]);
    }

    /**
     * @param   Auth\Simple  $as
     * @param   array        $options
     *
     * @return void
     * @throws Error\AuthSource
     */
    protected function handleLogin(Auth\Simple $as, array $options): void
    {
        $spSource = $as->getAuthSource();
        ...
        if(
            ...
            // We are already authenticated, so do nothing
        }

        // If a valid session exists, return
    ...
    /**
     * Map the Request parameters to option values
     *
     * @param   array  $requestParams  Associative array [parameter names => parameter value]
     *
     * @return array
     * @throws Error\Exception
     */
    protected function mapToOptionsList(array $requestParams): array
    {
        $mapper = [
            'target' => 'ReturnTo',
        ...
        foreach ($requestParams as $param => $value) {
            ...
            $paramName = $mapper[$param] ?? $param;
            $paramValue = ($paramName == 'ReturnTo') ? $httpUtils->checkURLAllowed($value) : $value;
    ...
    /**
     * Iterate over the parameter list and extract the values from the request. MAINTAIN the order
     *
     * @param   Request  $request           HTTP Request object
     * @param   array    $parameterList     An associative array [parameter name => isRequired]
     *
     * @return array
     * @throws BadRequest
     */
    protected function parseRequestParameters(Request $request, array $parameterList = []): array
    {
        $data = [];
        $hasReturnTo = false;
        foreach ($parameterList as $param => $required) {
            if (!$request->query->has($param) && (bool)$required) {
                if($param !== 'ReturnTo') {
                    throw new Error\BadRequest("Missing {$param} parameter.");
                }
                if($param === 'ReturnTo' || $param === 'target') {
                    $hasReturnTo = true;
                }
            }
        ...
        if($hasReturnTo && empty($data['target']) && empty($data['ReturnTo'])) {
