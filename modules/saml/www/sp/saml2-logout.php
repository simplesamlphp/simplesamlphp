...
if ($message instanceof LogoutResponse) {
    ...
    // Get consumer logout callbacks for this source.
    $session = \SimpleSAML\Session::getSessionFromRequest();
    $hasAsConsumerLogoutCallbacks = $source->hasAsConsumerLogoutCallbacks();
    $isProxyLogoutEnabled = $source->isProxyLogoutEnabled();
    $isFrontChannelBinding = ($binding instanceof \SAML2\HTTPPost || $binding instanceof \SAML2\HTTPRedirect);

    if ($isProxyLogoutEnabled && $hasAsConsumerLogoutCallbacks) {
        $state = [
            'saml:LogoutAssociations:authSourceId' => $sourceId,
            'saml:LogoutAssociations:logoutRequest' => $message,
            'saml:LogoutAssociations:idpEntityId' => $idpEntityId,
        ];

        $id = \SimpleSAML\Auth\State::saveState($state, 'asConsumerLogout:finishLogout');
        $returnToUrl = \SimpleSAML\Module::getModuleURL('saml/sp/saml2-logout-resume.php', ['id' => $id]);

        $source->callAsConsumerLogoutCallbacks($returnToUrl);
    }
    ...
    if ($isProxyLogoutEnabled && $hasAsConsumerLogoutCallbacks) {
        \SimpleSAML\Logger::warning(
            'Unable to log out all AS consumers. ' .
